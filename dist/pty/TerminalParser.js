...
export class TerminalParser {
    ...
    isCompleteCommand(command, tokens) {
        ...
        if (command.type === "OSC") {
            return ((lastToken.type === "CONTROL" && lastToken.value === "\x07") ||
                lastToken.type === "ST");
        }
        ...
        if (lastToken.value === "m") {
            ...
            if (tokens[0].type !== "ESC")
                return false;
            if (tokens[1].type !== "CSI")
                return false;
            ...
            for (let i = 2; i < tokens.length - 1; i++) {
                ...
                if (i % 2 === 0 && token.type !== "PARAMETER")
                    return false;
                if (i % 2 === 1 && token.type !== "PARAMETER_SEP")
                    return false;
            }
            return true;
        }
    ...
    *parse(input) {
        ...
        let currentCommand = this.incompleteCommand?.command || null;
        let textBuffer = "";
        ...
        for (const token of tokens) {
            ...
            switch (token.type) {
                ...
                case "ST": // Handle ST (String Terminator) token
                    if ((token.value === "\x07" || token.type === "ST") &&
                ...
                case "COMMAND":
                    if (token.value === "P" && currentCommand?.type === "UNKNOWN") {
                        ...
                        if (currentCommand) {
                            const command = this.parseCommand(currentCommand, token.value, token.raw);
                            if (command) {
                                yield command;
                            }
                            currentCommand = null;
    ...
    parseCommand(command, commandChar, rawCommandChar) {
        ...
        switch (commandChar) {
            ...
            case "c": // Device Attributes
                return {
                    type: "DEVICE",
                    action: params.length === 0 ? "primaryDA" : `deviceAttributes${params[0]}`,
                    params: params,
                    raw: fullRaw,
                };
            case "r":
    ...
    getNonPrivateModeAction(param, isSet) {
        const mode = isSet ? "set" : "reset";
        switch (param) {
            case 4:
    ...
    getCharsetAction(selector, charset) {
        const setName = {
            "(": "G0",
            ")": "G1",
            "*": "G2",
    ...
    parsePrivateCommand(command, commandChar, rawCommandChar) {
        const params = command.params || [];
        const fullRaw = (command.raw || "") + rawCommandChar;
    ...
    getPrivateModeAction(param, isSet) {
        const mode = isSet ? "set" : "reset";
        switch (param) {
            case 1:
