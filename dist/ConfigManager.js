...
class ConfigManager {
    ...
    async loadAndValidateCredentials(source) {
        try {
            let credentials;
            switch (source) {
                case "environment":
    ...
    async promptForManualCredentials() {
        console.log(chalk.yellow("\nPlease enter your AWS credentials:"));
        console.log(chalk.red("⚠️  Warning: Handle these credentials with care!\n"));
    ...
    async promptForCredentialStorage(credentials) {
        console.log(chalk.red("\n⚠️  Security Warning:"));
        console.log(chalk.yellow("Storing AWS credentials in config files is not recommended for security reasons.\n" +
        ...
        const { shouldStore } = await inquirer.prompt([
            {
                type: "confirm",
                name: "shouldStore",
                message: "Do you still want to save these credentials to the config file?",
                default: false,
            },
        ]);
        return shouldStore;
    ...
    async setupInitialConfig() {
        ...
        if (availableMethods.length > 0) {
            ...
            if (useExisting) {
                let selectedMethod = availableMethods[0];
                ...
                const validatedCreds = await this.loadAndValidateCredentials(selectedMethod);
                if (!validatedCreds) {
        ...
        const { region } = await inquirer.prompt([
            {
                type: "input",
                name: "region",
                message: "Which AWS region would you like to use? (Recommended: us-west-2)",
                default: "us-west-2",
            },
        ]);
        this.config.awsRegion = region;
        const bedrockManager = new BedrockManager(this.config, this.runtimeCredentials);
    ...
    async loadCredentials() {
        ...
        if (this.runtimeCredentials?.accessKeyId &&
        ...
        if (this.config.authMethod) {
            const credentials = await this.loadAndValidateCredentials(this.config.authMethod);
            if (credentials) {
                this.runtimeCredentials = credentials;
                return;
            }
            ...
            const { confirm } = await inquirer.prompt([
                {
                    type: "confirm",
                    name: "confirm",
                    message: chalk.yellow("⚠️  This will delete all saved configurations, credentials, and cached data. Are you sure?"),
                    default: false,
                },
            ]);
            if (!confirm) {
    ...
    async hasValidAccessAndCredentials() {
