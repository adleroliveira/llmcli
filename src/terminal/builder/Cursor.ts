...
export class Cursor {
  ...
  static moveTo(position: CursorPosition): SequenceProducer {
    return {
      toSequence: () =>
        CSISequence.create(CSICommand.CUP, [position.row, position.column]),
    };
  }
  ...
  static saveDEC(): SequenceProducer {
    return {
      toSequence: () => CSISequence.create(CSICommand.SM, [1048], true),
    };
  }

  static restoreDEC(): SequenceProducer {
    return {
      toSequence: () => CSISequence.create(CSICommand.RM, [1048], true),
    };
  }
