...
export class Text {
  ...
  static formatted(text: string, style: TextStyle): SequenceProducer {
    return {
      toSequence: () => {
        ...
        sequences.push(
          CSISequence.create(CSICommand.SGR, [SGRAttribute.Reset])
        );
  ...
  private static createFormatting(style: TextStyle): CSISequence[] {
    ...
    if (style.underline)
      sequences.push(
        CSISequence.create(CSICommand.SGR, [SGRAttribute.Underline])
      );
    if (style.blink)
      sequences.push(
        CSISequence.create(CSICommand.SGR, [SGRAttribute.BlinkSlow])
      );
    if (style.inverse)
      sequences.push(
        CSISequence.create(CSICommand.SGR, [SGRAttribute.Inverse])
      );
    if (style.hidden)
      sequences.push(CSISequence.create(CSICommand.SGR, [SGRAttribute.Hidden]));
    if (style.strikethrough)
      sequences.push(
        CSISequence.create(CSICommand.SGR, [SGRAttribute.StrikeThrough])
      );
    ...
    if (style.color !== undefined) {
      if (typeof style.color === "number" && style.color in SGRColor) {
        ...
        sequences.push(
          CSISequence.fromStr(TextFormatter.color256(style.color))
        );
      }
    }
    ...
    if (style.backgroundColor !== undefined) {
      if (
        typeof style.backgroundColor === "number" &&
        style.backgroundColor in SGRColor
      ) {
        sequences.push(
          CSISequence.create(CSICommand.SGR, [style.backgroundColor])
        );
