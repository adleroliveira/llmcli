...
export enum VerbosityLevel {
  ...
  DETAILED, // Everything including raw hex bytes
}
...
export class VT100Formatter {
  private static sequenceTypeNames = new Map(
    Object.entries(SequenceType)
  ...
  private static commandDescriptions = new Map([
  ...
  private static oscCommandDescriptions = new Map([
  ...
  private static escapeDescriptions = new Map([
    [
      "=",
  ...
  private static getOSCCommand(raw: Uint8Array): string | null {
    // Skip ESC ] at start
    let pos = 2;
    let cmd = "";
  ...
  private static formatText(raw: Uint8Array): string {
  ...
  private static getCommandDescription(
    ...
    if (sequence.type === SequenceType.C1 && sequence.raw.length === 1) {
      ...
      if (!desc) return `UNK_C1_${controlByte.toString(16)}`;
    ...
    if (sequence.type === SequenceType.ESCAPE && sequence.raw.length === 2) {
      ...
      if (!desc) return `UNK_ESC_${command}`;
      return verbosity === VerbosityLevel.MINIMAL ? desc.short : desc.full;
    }
    ...
    if (!desc) return `UNK_CMD_${command}`;

    // Check if this is a mode command (h or l) with parameters
    if (
      (command === "h" || command === "l") &&
      (sequence as any).parameters?.length > 0
    ) {
      const params = (sequence as any).parameters;
      ...
      if (firstParam.private) {
        ...
        if (modeDesc) {
          const baseDesc =
            verbosity === VerbosityLevel.MINIMAL ? desc.short : desc.full;
          const modeDescText =
  ...
  private static formatOSCData(raw: Uint8Array): string {
    ...
    let pos = 2;
    while (pos < raw.length && raw[pos] !== 0x3b) pos++;
    pos++; // Skip semicolon
    ...
    while (pos < raw.length) {
      const byte = raw[pos];
      if (byte === 0x07 || byte === 0x1b) break; // BEL or ESC (start of ST)
  ...
  static format(
    ...
    // Command
    parts.push(
      `<${this.getCommandDescription(sequence, finalOptions.verbosity)}>`
    );
    ...
    if (sequence.type === SequenceType.OSC) {
      ...
      const params = (sequence as any).parameters
        ? this.formatParameters((sequence as any).parameters)
        : "";
      if (params) {
        parts.push(`[${params}]`);
      }
    ...
    if (sequence.type === SequenceType.SGR) {
      const sgrSeq = sequence as SGRSequence;
