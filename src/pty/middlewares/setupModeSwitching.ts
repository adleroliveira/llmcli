import { Middleware, MiddlewareContext } from "../PtyStreamProcessor";
import { PtyManager } from "../PtyManager";
import { DebugLogger } from "../DebugLogger.js";
DebugLogger.initialize();

type ModeState = "terminal" | "ai";
...
export const createModeToggleMiddleware = (): Middleware => ({
  id: "mode-toggle",
  events: ["raw", "command"],
  transform: async (event, context: ExtendedContext) => {
    if (event.type === "raw" && event.content === "/" && !context.buffer) {
      const currentMode = context.metadata.mode || "terminal";
      const newMode: ModeState = currentMode === "terminal" ? "ai" : "terminal";
...
export const createPromptModifierMiddleware = (): Middleware => ({
  id: "prompt-modifier",
  events: ["line"],
  transform: async (event, context: ExtendedContext) => {
    ...
    const modifiedContent =
      event.content.slice(0, lastPromptPos + promptSymbol.length) +
      ` ${modeTag}` +
...
export const setupModeSwitching = (ptyManager: PtyManager) => {
