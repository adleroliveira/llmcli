...
describe("TerminalParser", () => {
  ...
  const parse = (input: string): ParsedCommand[] => {
    return Array.from(parser.parse(input));
  };

  const removeDebug = (input: ParsedCommand): ParsedCommand => {
  ...
  describe("Basic Parsing", () => {
    ...
    test("handles CRLF sequence correctly", () => {
      ...
      expect(
        commands.map((cmd) => ({ type: cmd.type, action: cmd.action }))
      ).toEqual([
  ...
  describe("Complex Sequences", () => {
    ...
    test("handles complex prompt sequence", () => {
      const input =
        "\x1B[1m\x1B[7m%\x1B[m\x1B[1m\x1B[m Zartharus% \x1B[K\x1B[?1h=\x1B[?2004h";
      const commands = parse(input);
