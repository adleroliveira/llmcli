...
// Parser states
enum ParserState {
  GROUND = "GROUND", // Not currently parsing a sequence
  ESCAPE = "ESCAPE", // Seen ESC, waiting for next char
  CSI = "CSI", // Parsing CSI sequence
  OSC = "OSC", // Parsing OSC sequence
...
interface ParserOptions {
  // Whether to accept 8-bit C1 controls
  support8BitC1: boolean;
  // Maximum length for string sequences (OSC/DCS)
...
export class VT100Parser {
  ...
  private surrogatePending: number | null = null;
  private savedState: {
    state: ParserState;
    params: string[];
   ...
   */
  public stringToBytes(input: string): Uint8Array {
    ...
    while (i < input.length) {
      ...
      if (["\n", "\r", "\t", "\b", "\f", "\v", "\0"].includes(char)) {
        ...
        bytes.push(controlBytes[char as keyof typeof controlBytes]);
        i++;
        continue;
      }
   ...
   */
  static hasEscapeSequences(input: string): boolean {
    return (
      input.includes("\x1b") ||
      input
        .split("")
        .some(
          (char) => char.charCodeAt(0) >= 0x80 && char.charCodeAt(0) <= 0x9f
        )
    );
  }

  private flushTextBuffer(): TextSequence | null {
          ...
          if (this.textBuffer.length === this.savedState.expectedBytes) {
            ...
            try {
              new TextDecoder("utf-8", { fatal: true }).decode(
                new Uint8Array(this.textBuffer)
              );
              this.savedState = null;
            } catch (e) {
    ...
    switch (byte) {
      ...
      default:
        ...
        if (byte >= 0x30 && byte <= 0x7e) {
          const sequence = new SimpleEscapeSequence(
        ...
        if (this.options.strictMode) {
          throw new Error(
            `Invalid escape sequence byte: ${byte} (0x${byte.toString(16)})`
          );
        }
    ...
    if (byte === ControlCharacter.ESC) {
      ...
      if (this.buffer.length > 2) {
        ...
        const sequence = new CSISequence(
          new Uint8Array(this.buffer.slice(0, -1)), // exclude the ESC
          params,
          this.intermediates,
          this.buffer[this.buffer.length - 2] // use last byte before ESC as final
        );
        this.reset();
        this.state = ParserState.ESCAPE;
        this.buffer = [byte];
  ...
  private processDCS(byte: number): VT100Sequence | null {
    ...
    if (byte === ControlCharacter.ST) {
      const params = this.parseParams();
      const sequence = new DCSSequence(
        new Uint8Array(this.buffer),
        params,
        this.intermediates,
        this.stringContent,
  ...
  private processCharset(byte: number): VT100Sequence | null {
    ...
    const sequence = new CharsetSequence(
      new Uint8Array(this.buffer),
      designator as CharsetDesignator,
      byte
    );
    this.reset();
    return sequence;
