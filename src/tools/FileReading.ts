...
export const createFileReadTool = () => {
  return new ToolBuilder("readFile")
    .describe(
      "Reads and returns the contents of a file in the current directory"
    )
    .input(
      "filename",
      "string",
      "Name of the file to read (must be in current directory)",
      true
    )
...
export const createFileWriteTool = () => {
  return new ToolBuilder("writeFile")
    .describe(
      "Creates a new file or overwrites an existing file in the current directory"
    )
