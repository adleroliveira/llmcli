...
export const createDisplayDirectoryTool = () => {
  return new ToolBuilder("displayFiles")
    .describe(
      "Displays files and directories in the current working directory to screen"
    )
    .input(
      "path",
      "string",
      "Relative path to list files from (defaults to current directory)",
      false
    )
    .input(
      "showHidden",
    ...
    .handle(async ({ path = ".", showHidden = false }) => {
      try {
        ...
        fileList.forEach((file) => {
          const sizeStr =
            file.type === "directory" ? "<DIR>" : `${file.size} bytes`;
          console.log(
            `${file.type === "directory" ? "📁" : "📄"} ${file.name.padEnd(
              30
            )} ${sizeStr.padEnd(15)} ${file.modifiedAt}`
          );
        });
