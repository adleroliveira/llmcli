...
import { CliConfig, AwsCredentials } from "./ConfigManager";
...
export class BedrockManager {
  ...
  constructor(config: CliConfig, credentials: AwsCredentials) {
    this.config = config;
  ...
  private async checkConverseAccess(modelId: string): Promise<boolean> {
  ...
  public async setupBedrockConfig(): Promise<void> {
    ...
    console.log(
      chalk.green(
        `\n✅ Successfully configured Bedrock with model: ${this.formatModelName(
          modelId
        )}`
      )
    );
  ...
  public async validateModelAccess(): Promise<boolean> {
    try {
